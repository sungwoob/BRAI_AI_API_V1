...
FastAPI 기반으로 작성된 BRAI 백엔드 API 모의 서버입니다. `spec/API_SPECIFICATION_v1.0.md`(문서 버전 1.0 기준)에 정의된 주요 엔드포인트를 구현하며, 메모리 내 가짜 데이터를 반환합니다. 초기 실행 시 명세서 예시와 동일한 예측 레코드와 조합 ID를 시드 데이터로 준비합니다.
