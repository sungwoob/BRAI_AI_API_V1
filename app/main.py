...
app = FastAPI(title="BRAI API", version="1.0.0")
...
def create_prediction(request: PredictionRequest):
    male_id = request.maleStrainId.strip()
    female_id = request.femaleStrainId.strip()
    ...
    male = data.get_strain(male_id)
    female = data.get_strain(female_id)
    ...
    if not prediction:
        prediction = data.create_prediction(
            male,
            female,
            generate_images=request.options.generateImage,
        )

    return JSONResponse(
        {"success": True, "data": prediction.dict(), "message": "예측이 성공적으로 완료되었습니다"}
    )
...
def get_predictions(
    ...
    total = len([p for key, p in data.PREDICTIONS.items() if "-" not in key])
